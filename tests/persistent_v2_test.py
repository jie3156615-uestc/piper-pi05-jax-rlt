...
from openpi.rlt.real.actor_runtime import FRESH_ZERO_ACTOR_INITIALIZATION
from openpi.rlt.real.actor_runtime import FRESH_ZERO_LINEAGE_MANIFEST_FORMAT
...
def test_actor_runtime_loads_legacy_v3_and_persistent_v4_but_outputs_only_v3(tmp_path):
    ...
    legacy_metadata = legacy_actor.provenance_metadata()
    assert legacy_metadata["actor_checkpoint_action_schema_fingerprint"] == ACTION_SCHEMA_FINGERPRINT
    assert legacy_metadata["actor_output_protocol_action_schema_fingerprint"] == ACTION_SCHEMA_FINGERPRINT
    assert legacy_metadata["actor_checkpoint_kind"] == "legacy_v3_rank1_source"
...
def _fresh_zero_gripper_close_learner():
    ...
    replay = _persistent_replay()
    replay["action_schema_fingerprint"] = np.asarray(
        [PERSISTENT_GRIPPER_CLOSE_ACTION_SCHEMA_FINGERPRINT] * len(replay["reward"])
    )
    for name, value in {
        "execution_gripper_residual_max_close_m": cfg.actor_gripper_residual_max_close_m,
        "execution_gripper_d1_max_m": cfg.actor_gripper_residual_d1_max_m,
        ...
        replay[name] = np.full((len(replay["reward"]), 10), value, dtype=np.float32)
    fingerprints = {
        **{
            key: value
            for key, value in EXPECTED_CHECKPOINT_FINGERPRINTS.items()
        ...
        "actor_initialization": FRESH_ZERO_ACTOR_INITIALIZATION,
        "fresh_zero_lineage_manifest_format": FRESH_ZERO_LINEAGE_MANIFEST_FORMAT,
        "fresh_zero_lineage_manifest_sha256": "a" * 64,
    }
    learner = RealRLTLearner.create(
        replay,
        config=cfg,
    ...
    learner.update_step = learner.config.actor_start_step + 1
    return learner


def test_actor_runtime_loads_explicit_fresh_zero_gripper_close_checkpoint(tmp_path):
    learner = _fresh_zero_gripper_close_learner()
    checkpoint = tmp_path / "fresh_zero"
    learner.save_checkpoint(checkpoint)

    actor = JaxCheckpointShadowActor(checkpoint)

    assert actor.provenance_metadata()["actor_checkpoint_kind"] == (
        "persistent_v5_gripper_close_fresh_zero_rank1_output"
    )


def test_actor_runtime_rejects_gripper_close_without_initialization_provenance(tmp_path):
    learner = _fresh_zero_gripper_close_learner()
    learner.fingerprints.pop("actor_initialization")
    learner.fingerprints.pop("fresh_zero_lineage_manifest_format")
    learner.fingerprints.pop("fresh_zero_lineage_manifest_sha256")
    checkpoint = tmp_path / "missing_provenance"
    learner.save_checkpoint(checkpoint)

    with pytest.raises(ValueError, match="explicit fresh-zero initialization"):
        JaxCheckpointShadowActor(checkpoint)


def test_actor_runtime_rejects_conflicting_fresh_zero_and_warm_start_provenance(tmp_path):
    learner = _fresh_zero_gripper_close_learner()
    learner.fingerprints["warm_start_actor_source_schema"] = ACTION_SCHEMA_FINGERPRINT
    checkpoint = tmp_path / "conflicting_provenance"
    learner.save_checkpoint(checkpoint)

    with pytest.raises(ValueError, match="conflicting warm-start and fresh-zero"):
        JaxCheckpointShadowActor(checkpoint)
